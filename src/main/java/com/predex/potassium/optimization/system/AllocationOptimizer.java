...
public final class AllocationOptimizer {
    ...
    public static final class IntBufferPool {
        ...
        public IntBufferPool(int max) { this.max = Math.max(1, max); }

        public int[] acquire(int size) {
            int[] value = pool.pollFirst();
            if (value == null || value.length < size) return new int[size];
            return value;
        }

        public void release(int[] value) {
            if (value != null && pool.size() < max) pool.offerFirst(value);
        }
    }
}